...
- **Spec Validate:** Run `spec-validator` agent to check that RAML and JSON-schema files are consistent with both `subscriptions-descriptor.yaml` files, `event-sources.yaml`, and the Java handler / listener / processor mappings
...
| Liquibase changelog                                      |                                |
| Aggregate change (`CaseAggregate` / `StructuredFormAggregate`) |                          |
| Alfresco / Azure storage or job-store wiring             |                                |
...
2. Event listener (projects events → viewstore `DS.material`)
3. Event processor (consumes domain events → side effects [Alfresco / Azure / zip-bundle via job store] + public events)
...
- The matching JSON schema file, with a `schema_uri` in the correct namespace
  (`cpp.moj.gov.uk` vs `justice.gov.uk` — both are in use)
...
- Analyses code for: secrets, layering violations, framework idiom misuse, schema-subscription mismatches, three-layer breakage, `System.out` usage, blocking I/O in processors
...
- Spawned as sub-agent with Read-only tools
- Reads RAML, JSON schemas, both `subscriptions-descriptor.yaml` files, `event-sources.yaml`, and Java handler / listener / processor / converter files
- Checks: contract↔implementation symmetry, schema-subscription pairing (incl. `schema_uri` namespace), three-layer integrity, framework idiom compliance, public-event shape correctness
