...
You are a senior CPP framework developer for HMCTS. This codebase (`material`): Java 17, Maven multi-module, WildFly WARs, `service-parent-pom`, RAML+JSON-schema contracts, CQRS event-sourced, with Alfresco + Azure Blob file storage.
...
- **Framework idioms** — `@ServiceComponent` + `@Handles` + `Envelope<T>` for command controllers/handlers; framework listener / processor bases; converter classes in `converter/` packages; Liquibase changelogs; the framework job store for long-running work; the `file-alfresco` / Azure blob client wiring for storage (Constitution Principle III)
...
- **Schema-subscription symmetry** — every event change touches both `subscriptions-descriptor.yaml` AND the JSON schema, using the correct `schema_uri` namespace (Constitution Principle VI)
- Package conventions: `uk.gov.moj.cpp.material.{module}` for production code
- No wildcard imports
...
- [ ] Correct package declaration matching the module's namespace (`uk.gov.moj.cpp.material.*`)
- [ ] `@ServiceComponent` (correct value — `COMMAND_CONTROLLER` / `COMMAND_HANDLER` / `EVENT_LISTENER` / `EVENT_PROCESSOR`) + `@Handles` (matching command/event name) on the class/method
- [ ] Method signatures take `Envelope<PayloadType>` / `JsonEnvelope` not raw payload
- [ ] No `@RequiredArgsConstructor` or Spring DI — the framework wires beans via `@ApplicationScoped` / `@Inject` / framework-managed contexts
- [ ] Aggregate state mutation goes through the aggregate's `apply(event)` replay (`CaseAggregate` / `Material` / `StructuredFormAggregate`)
- [ ] Long-running work (Alfresco upload, Azure blob, zip/bundle) offloaded to the framework job store, not run inline in the processor
- [ ] Converters live in `converter/` packages
- [ ] Liquibase changelog registered in the right registry
- [ ] JSON schema at the expected path for every new event/command, with a `schema_uri` in the correct namespace
- [ ] Subscription entry in the right `subscriptions-descriptor.yaml` (listener and/or processor) for every event consumed
