...
/**
 * JNDI-backed configuration for the archive Azure Blob container.
 *
 * <p>Used by the case archiving pipeline to store ZIP archives.
 */
@SuppressWarnings("java:S6813")
@ApplicationScoped
public class ArchiveAzureBlobConfiguration {

    @Inject
    @Value(key = "material.archive.storage.connection-string", defaultValue = "DefaultAzureCredential")
    private String connectionString;

    @Inject
    @Value(key = "material.archive.storage.endpoint")
    private String endpoint;

    @Inject
    @Value(key = "material.archive.storage.container-name")
    private String containerName;

    @Inject
    @Value(key = "material.archive.storage.connection-timeout-seconds", defaultValue = "10")
    private String connectionTimeoutSeconds;

    @Inject
    @Value(key = "material.archive.storage.response-timeout-seconds", defaultValue = "30")
    private String responseTimeoutSeconds;

    @Inject
    @Value(key = "material.archive.storage.transfer-timeout-seconds", defaultValue = "300")
